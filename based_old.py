...
def compute_per_class_metrics(
    cm: np.ndarray, label_lookup: dict[int, str]
) -> list[dict]:
    total = cm.sum()
    metrics = []
...
def run_cross_validation(train_indices: np.ndarray) -> dict:
    ...
    for fold_tr, fold_te in skf.split(train_indices, y_sub[train_indices]):
        ...
        y_true = y_sub[te_idx]
        y_pred = preds["top1"]
        macro_all.append(f1_score(y_true, y_pred, average="macro", zero_division=0.0))
        mask = ~preds["handoff"]
        if mask.any():
            macro_conf.append(
                f1_score(y_true[mask], y_pred[mask], average="macro", zero_division=0.0)
            )
        else:
...
def run_report(val_size: float = 0.2, random_state: int = RANDOM_STATE) -> Path:
    ...
    report_dir = Path("runs") / "reports" / datetime.utcnow().strftime("%Y%m%d-%H%M%S")
